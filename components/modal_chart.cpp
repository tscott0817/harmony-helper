...
//                           chartCenter, 0, WHITE);
            ...
            if (!chartLocAdded) {
                chartLocations[i][j] = {static_cast<float>(((container.x + (container.x * 0.0025f)) + (container.width * .142857f) * i) - (container.width / 2)), static_cast<float>((container.y - (container.y * .29f)) - (container.height * .16f) * j) + (container.height / 2)};
            }
        }
    }
